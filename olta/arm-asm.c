...
static void _str_ind_by_size(asm_ctx_t *ctx, int size, reg_t src_r, reg_t dst_addr_r, int offset_idx, int scale) {
    uint32_t base = 0;
    switch (size) {
        case 1: base = 0x38000000; break;
        case 2: base = 0x78000000; break;
        case 4: base = 0xb8000000; break;
        case 8: base = 0xf8000000; break;
        default: assert(0);
    }
    if ((offset_idx < 0) || (offset_idx > 0 && scale == 0)) {
        assert(scale == 0);
...
static void _str_ind(asm_ctx_t *ctx, reg_t src_r, reg_t dst_addr_r) {
    _str_ind_by_size(ctx, 8, src_r, dst_addr_r, 0, 0);
}
...
static int build_str(asm_ctx_t *ctx, ins_desc_t *desc) {
    if ((desc->flags & I_INDIRECT) && ((desc->n_arg == 2) || (desc->n_arg == 3))) {
        ...
        if (size == 1 || size == 2 || size == 4 || size == 8) {
            if (desc->n_arg == 2) {
                _str_ind_by_size(ctx, size, src_a->n, addr_a->n, 0, 0);
                return 0;
            } else if (desc->n_arg == 3) {
                ins_arg_t *offset_a = &(desc->arg[2]);
                if (desc->flags & I_OFFSET_CONST) {
                    // XXX: note, not scaled, this seems to be the expected behaviour
                    _str_ind_by_size(ctx, size, src_a->n, addr_a->n, offset_a->n, 0);
                    return 0;
                } else if (desc->flags & I_OFFSET_REG) {
                    // XXX: note, not scaled, this seems to be the expected behaviour
